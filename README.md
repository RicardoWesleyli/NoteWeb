...
- **⚙️ 强大的数据管理**：
  - **可视化编辑**：提供直观的表格界面管理分类和链接，操作简单便捷。
  - **🔍 智能搜索**：支持在分类管理中快速搜索和筛选，高效管理大量数据。
  - **📝 双模式支持**：同时提供可视化表格编辑和 **JSON 代码编辑**，满足不同用户需求。
  - **🌍 国际化界面**：管理面板完全适配中英文界面，无缝切换。
...
你可以在网页右上角的 **设置** 图标中打开管理面板，体验全新升级的数据编辑器：

1. **可视化管理**：
   - **分组管理**：支持查看、添加、编辑和删除分类。新增**搜索功能**，可快速定位特定分类。
   - **链接管理**：直观地管理各个分类下的链接资源。
   - **界面优化**：搜索框和输入框自动适配屏幕宽度，提供更好的操作体验。

2. **高级模式**：
   - 保留了 **JSON 编辑器**，支持高级用户直接修改底层数据结构，批量处理更高效。

3. **国际化支持**：
   - 编辑器界面文案跟随系统语言设置（中文/英文），提供原生的使用体验。

所有修改会立即生效并自动保存到浏览器本地存储（LocalStorage）。
